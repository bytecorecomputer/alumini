...
const targetPath = path.join(__dirname, 'src', 'data', 'hindiQuizData.js');
let content = fs.readFileSync(targetPath, 'utf8');

// The new modules to append before the final '};'
const newModules = `
    "libre_writer": {
...
// Regex to inject notes into ms_word
const wordNotes = \`"notes": \\\`<h2>MS Word: Complete Master Guide & Shortcut Keys</h2>
...
// Regex to inject notes into ms_excel (100+ formulas)
const excelNotes = \`"notes": \\\`<h2>MS Excel: 100+ Formulas & Master Guide</h2>
...
// Replace logic
// Find "ms_word": { ... "modules": {
content = content.replace(/"ms_word":\s*\{\s*"title"/, '"ms_word": {\n        "title"');
content = content.replace(/"ms_word":\s*\{\s*"title"/, '"ms_word": {\n        ' + wordNotes + '\n        "title"');

content = content.replace(/"ms_excel":\s*\{\s*"title"/, '"ms_excel": {\n        ' + excelNotes + '\n        "title"');

// Append new modules before last };
content = content.replace(/};\s*$/, ',\n' + newModules + '\n};');

fs.writeFileSync(targetPath, content);
console.log('Successfully injected content into hindiQuizData.js');
